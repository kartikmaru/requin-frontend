  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
      <div className="bg-white rounded-2xl shadow-sm p-6 mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl sm:text-2xl font-bold text-slate-800">Welcome, {user?.name} 👋</h2>
          <p className="text-slate-500 text-sm mt-1">Your personal dashboard</p>
        </div>
        {user?.role === "admin" && (
          <button
            onClick={() => navigate("/admin/dashboard")}
            className="flex items-center gap-2 px-5 py-2.5 bg-slate-900 hover:bg-slate-700 text-white font-bold text-sm rounded-lg shadow-md transition-colors whitespace-nowrap self-start sm:self-auto"
          >
            🛡️ Admin Panel
          </button>
        )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <Link to="/products" className="block no-underline">
          <div className="bg-white rounded-2xl shadow-sm p-6 text-center hover:-translate-y-1 hover:shadow-md transition-all cursor-pointer h-full">
        ...
        <Link to="/my-orders" className="block no-underline">
          <div className="bg-white rounded-2xl shadow-sm p-6 text-center hover:-translate-y-1 hover:shadow-md transition-all cursor-pointer h-full">
          ...
          <table className="w-full text-sm">
            <tbody>
              <tr className="border-b border-slate-100">
                <td className="py-2.5 pr-4 font-semibold text-slate-600 w-20">Name</td>
                <td className="py-2.5 text-slate-800">{user?.name}</td>
              </tr>
              <tr className="border-b border-slate-100">
                <td className="py-2.5 pr-4 font-semibold text-slate-600">Email</td>
                <td className="py-2.5 text-slate-800 break-all">{user?.email}</td>
              </tr>
              <tr>
                <td className="py-2.5 pr-4 font-semibold text-slate-600">Role</td>
                <td className="py-2.5">
                  <span className={`inline-block px-2.5 py-0.5 rounded-full text-xs font-semibold ${user?.role === "admin" ? "bg-indigo-100 text-indigo-700" : "bg-green-100 text-green-700"}`}>
